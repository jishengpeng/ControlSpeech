...
class RecognizerInfer:
    ...
    def example_run(cls): 
        ...
        for item in tqdm(test_items):
            ...
            item_name = process_item_name(item_name)
            emo_embed = np.load(f'./wav_promptself_seq/{item_name}.npy')
...
def process_item_name(item_name):
    if item_name.startswith('CREMA-D'):
        item_name = item_name.split('-',maxsplit=3)[-1]
    if item_name.startswith('Emotional'):
        item_name = item_name.split('-',maxsplit=4)[-1]
    if item_name.startswith('RAVDESS'):
        item_name = item_name.split('-',maxsplit=2)[-1]
    if item_name.startswith('TESS'):
        item_name = item_name.split('-',maxsplit=3)[-1]
    if item_name.startswith('MESS'):
        item_name = item_name.split('-',maxsplit=1)[-1]
    if item_name.startswith('SAVEE'):
        item_name = item_name.split('-',maxsplit=2)[-1]
    return item_name
