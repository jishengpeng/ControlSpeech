...
from modules.tts.prodiff_adapt.model.fs import FastSpeech, FastSpeech_ada4,FastSpeech_ada4_spk1,FastSpeech_ada4_spk2,FaseSpeech_multiscene, FastSpeech_multiscene_ref,FaseSpeech_scene_baseline,FastSpeech_gener,FastSpeech_multiscene_ref_lstm,FastSpeech_multiscene_fine_grained
...
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, mel, cond, infer=False):
        b, *_, device = *mel.shape, mel.device
        nonpadding = (mel.abs().sum(dim=-1)>0).float()
        nonpadding = (ret['mel2ph'] != 0).float().unsqueeze(1).unsqueeze(1) # [B, T]
        cond = cond.transpose(1, 2)  # [B,C,T]
        if not infer:
