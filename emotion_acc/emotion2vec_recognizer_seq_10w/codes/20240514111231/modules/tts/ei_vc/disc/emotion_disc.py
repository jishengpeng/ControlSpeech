...
class EmotionClassifier(nn.Module):
    def __init__(self,input_dim=256,output_dim=8):
...
class Emotion_Discriminator(nn.Module):
    def __init__(self, mel_dim=80,out_dim=8):
        super(Emotion_Discriminator, self).__init__()
        # encoder
        self.encoder = ECAPA_TDNN_SMALL(mel_dim, emb_dim=256)
        self.head = EmotionClassifier(input_dim=256,output_dim=out_dim)
    
    def extract_emo(self,mels):
        ### mels [B,T,C]
...
class Emotion2Vec_finetume(nn.Module):
...
class Emotion2Vec_finetume_ori(nn.Module):
    def __init__(self, ssl_dim=768,out_dim=8):
        super(Emotion2Vec_finetume_ori, self).__init__()
        self.head = EmotionClassifier(input_dim=ssl_dim,output_dim=out_dim)
        
    def forward(self,ssl_features):
        ### ssl_features [B,C]
        ret = {}
        logits = self.head(ssl_features)
        ret['logits'] = logits
        return ret
