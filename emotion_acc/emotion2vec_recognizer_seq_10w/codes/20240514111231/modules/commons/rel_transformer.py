    ...
    def __init__(self, hidden_channels, filter_channels, n_heads, n_layers, kernel_size=1, p_dropout=0.,
        ...
        for i in range(self.n_layers):
            self.attn_layers.append(
                MultiHeadAttention(hidden_channels, hidden_channels, n_heads, window_size=window_size,
                                   p_dropout=p_dropout, block_length=block_length))
            self.norm_layers_1.append(Conditional_LayerNorm(hidden_channels))
            self.ffn_layers.append(
                FFN(hidden_channels, hidden_channels, filter_channels, kernel_size, p_dropout=p_dropout))
            self.norm_layers_2.append(Conditional_LayerNorm(hidden_channels))
        if pre_ln:
            self.last_ln = Conditional_LayerNorm(hidden_channels)

    def forward(self, x,spk_embed, x_mask):
        attn_mask = x_mask.unsqueeze(2) * x_mask.unsqueeze(-1)
        for i in range(self.n_layers):
            x = x * x_mask
            x_ = x
            if self.pre_ln:
                x = self.norm_layers_1[i](x,spk_embed)
            y = self.attn_layers[i](x, x, attn_mask)
            y = self.drop(y)
            x = x_ + y
            if not self.pre_ln:
            ...
            x_ = x
            if self.pre_ln:
                x = self.norm_layers_2[i](x,spk_embed)
            y = self.ffn_layers[i](x, x_mask)
            y = self.drop(y)
            x = x_ + y
            if not self.pre_ln:
...
class ConditionalRelTransformerEncoder(nn.Module):
    def __init__(self,
                 n_vocab,
                 in_channels,
                 out_channels,
    ...
    def forward(self, x,spk_embed, x_mask=None):
        if self.n_vocab > 0:
            x = self.emb(x) * math.sqrt(self.in_channels)  # [b, t, h]
        x = torch.transpose(x, 1, -1)  # [b, h, t]
        # x_mask = torch.unsqueeze(sequence_mask(x_lengths, x.size(2)), 1).to(x.dtype)
        x = self.proj(x) * x_mask
        x = self.pre(x, x_mask)
        x = self.encoder(x, spk_embed,x_mask)
        return x.transpose(1, 2)