...
from tasks.tts.vc_editor_dataset_utils import RecognizerDataset



class TimbreDiscSpeechTask(FastSpeechTask):
    def __init__(self):
        super(TimbreDiscSpeechTask, self).__init__()
    ...
    def build_tts_model(self):
        self.model = Timbre_Discriminator(hparams['audio_num_mel_bins'],hparams['spk_num'])
        models = [self.model]
        self.model_params = []
        for model in models:
    ...
    def run_model(self, sample, infer=False):
        ...
        if not infer:
            ...
            losses = {}
            self.add_cls_loss(output['logits'], spk,losses=losses)
            accuracy = self.compute_accuracy(output['logits'].detach(), spk)
            output['acc'] =  accuracy
    ...
    def add_cls_loss(self,pred,tgt_label,losses=None):
        spk_cls_loss = self.criterion(pred,tgt_label)
        losses['spk_cls_loss'] = spk_cls_loss
