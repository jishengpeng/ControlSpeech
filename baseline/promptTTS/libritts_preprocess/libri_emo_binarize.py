...
class BaseBinarizer:
    tg_fn_map = {}
    def __init__(self, processed_data_dir='data/processed/libritts_emo', binary_data_dir='data/binary/libritts_emo'):
    ...
    def load_meta_data(self):
        ...
        # total_tg_fns = glob.glob(f"{processed_data_dir}/mfa_outputs/**/*.TextGrid",recursive=True) + \
        #     glob.glob('/data1/pmy/jlpro/EI_VC/data/processed/emo_mfa_outputs/**/*.TextGrid',recursive=True)
        total_tg_fns = glob.glob(f"{processed_data_dir}/mfa_outputs/**/*.TextGrid",recursive=True)
        for tg_fn in tqdm(total_tg_fns):
            item_name = os.path.basename(tg_fn).split('.')[0]
            BaseBinarizer.tg_fn_map[item_name] = tg_fn
        for r in tqdm(items_list, desc='Loading meta data.'):
            item_name = r['item_name']
            spk_name = item_name.split('_')[0]
            num = 0 
            tg_fn =  BaseBinarizer.tg_fn_map.get(item_name)
            if tg_fn is None or (not os.path.exists(tg_fn)):
                num += 1
                continue
        ...
        print(num,len(self.items))

    @property
    def train_item_names(self):
        range_ = self._convert_range([20, -1])
        return self.item_names[range_[0]:range_[1]]

    @property
    def valid_item_names(self):
        range_ = self._convert_range([0, 20])
        return self.item_names[range_[0]:range_[1]]
    ...
    def process_item(cls, item, text2mel_params):
        ...
        try:
            ...
            if text2mel_params['with_align']:
                spk_name = item_name.split('_')[0]
                tg_fn = BaseBinarizer.tg_fn_map[item_name]
                item['tg_fn'] = tg_fn
            ...
            if text2mel_params['with_f0']:
                cls.process_pitch(item, text2mel_params,wav)
    ...
    def process_audio(cls, wav_fn, res, text2mel_params):
        ...
        res.update({'mel': mel,'sec': len(wav) / text2mel_params['audio_sample_rate'], 'len': mel.shape[0]})
        return wav, mel
    ...
    @staticmethod
    def process_pitch(item, text2mel_params, wav):
        mel = item['mel']
