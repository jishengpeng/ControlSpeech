...
from modules.commons.layers import LayerNorm, Embedding, Conditional_LayerNorm
...
class DecSALayer(nn.Module):
    def __init__(self, c, num_heads, dropout, attention_dropout=0.1, relu_dropout=0.1,
        ...
        self.layer_norm3 = Conditional_LayerNorm(c)
        self.ffn = TransformerFFNLayer(
    ...
    def forward(
        ...
        residual = x
        x = self.layer_norm3(x, spk_embed)
...
class ConditionalTransformerEncoderLayer(nn.Module):
    def __init__(self, hidden_size, dropout, kernel_size=9, num_heads=2):
        ...
        self.dropout = dropout
        self.num_heads = num_heads
        self.op = ConditionalEncSALayer(
            hidden_size, num_heads, dropout=dropout,
            attention_dropout=0.0, relu_dropout=dropout,
            kernel_size=kernel_size)

    def forward(self, x, spk_emb, **kwargs):
        return self.op(x, spk_emb, **kwargs)
...
class ConditionalFFTBlocks(nn.Module):
    def __init__(self, hidden_size, num_layers, ffn_kernel_size=9, dropout=0.0,
                 num_heads=2, use_pos_embed=True, use_last_norm=True,
                 use_pos_embed_alpha=True):
        super().__init__()
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([
            ConditionalTransformerEncoderLayer(self.hidden_size, self.dropout,
                                    kernel_size=ffn_kernel_size, num_heads=num_heads)
            for _ in range(self.num_layers)
        ])
        if self.use_last_norm:
            self.layer_norm = Conditional_LayerNorm(embed_dim)
        else:
            self.layer_norm = None

    def forward(self, x, spk_embed, padding_mask=None, attn_mask=None, return_hiddens=False):
        """
        :param x: [B, T, C]
        :param padding_mask: [B, T]
        :return: [B, T, C] or [L, B, T, C]
        ...
        hiddens = []
        for layer in self.layers:
            x = layer(x, spk_embed, encoder_padding_mask=padding_mask, attn_mask=attn_mask) * nonpadding_mask_TB
            hiddens.append(x)
        if self.use_last_norm:
            x = self.layer_norm(x, spk_embed) * nonpadding_mask_TB
        if return_hiddens:
            x = torch.stack(hiddens, 0)  # [L, T, B, C]
            x = x.transpose(1, 2)  # [L, B, T, C]
        else:
...
class ConditionalFastSpeechEncoder(ConditionalFFTBlocks):
    def __init__(self, dict_size, hidden_size=256, num_layers=4, kernel_size=9, num_heads=2,
                 dropout=0.0):
        super().__init__(hidden_size, num_layers, kernel_size, num_heads=num_heads,
                         use_pos_embed=False, dropout=dropout)  # use_pos_embed_alpha for compatibility
    ...
    def forward(self, txt_tokens,spk_embed, attn_mask=None):
        """
        ...
        x = self.forward_embedding(txt_tokens)  # [B, T, H]
        if self.num_layers > 0:
            x = super(ConditionalFastSpeechEncoder, self).forward(x,spk_embed, encoder_padding_mask, attn_mask=attn_mask)
        return x
...
class ConditionalFastSpeechDecoder(ConditionalFFTBlocks):
    def __init__(self, hidden_size=256, num_layers=4, kernel_size=9, num_heads=2):
        super().__init__(hidden_size, num_layers, kernel_size, num_heads=num_heads)
