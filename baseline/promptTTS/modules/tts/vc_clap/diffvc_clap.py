...
from modules.tts.ProDiff.model.diff.shallow_diffusion_tts import *
import numpy as np
import torch
...
from modules.tts.vc_clap.fsvc_clap import VC_Network,VC_Seq_Network,\
                        TTS_Seq_Network,TTC_Network,Codec_WavLM_Network,TTS_Multi_Stream_Network



class Diffusion_VC_CLAP(nn.Module):
    def __init__(self, out_dims, denoise_fn,
        ...
        self.vc_clap_encoder = VC_Network(hparams, out_dims)
    ...
    def cwt2f0_norm(self, cwt_spec, mean, std, mel2ph):
        return self.fs.cwt2f0_norm(cwt_spec, mean, std, mel2ph)

    def out2mel(self, x):
        return x

    def forward(self, mel, ref_mel, ph_token, style_embed, mel2ph=None,
                f0=None, uv=None, infer=False,ifreturn=False):
        b, *_, device = *ph_token.shape, ph_token.device
        target = mel
        ret = self.vc_clap_encoder(mel, ref_mel, ph_token, style_embed, mel2ph, f0, uv, infer=infer)
        if(ifreturn):
            return ret
        ...
        if not infer:
            ...
            # Diffusion
            x_t = self.diffuse_fn(target, t) * nonpadding
...
class NS2_VC_CLAP(nn.Module):
    def __init__(self, out_dims, denoise_fn,
                 timesteps=1000, time_scale=1, loss_type='l1', betas=None, spec_min=None, spec_max=None):
        super().__init__()
        self.denoise_fn = denoise_fn
        self.vc_clap_encoder = VC_Seq_Network(hparams, out_dims)
        self.mel_bins = out_dims
...
class NS2_TTS_CLAP(nn.Module):
    def __init__(self,phone_encoder, out_dims, denoise_fn,
                 timesteps=1000, time_scale=1, loss_type='l1', betas=None, spec_min=None, spec_max=None):
        super().__init__()
        self.denoise_fn = denoise_fn
        self.vc_clap_encoder = TTS_Multi_Stream_Network(len(phone_encoder),hparams, out_dims)
        self.mel_bins = out_dims
    ...
    def forward(self, mel, ref_mel, ph_token, style_embed, mel2ph=None,
                f0=None, uv=None, infer=False,ifreturn=False,global_step=0):
        b, *_, device = *ph_token.shape, ph_token.device
        target = mel
        ret = self.vc_clap_encoder(mel, ref_mel, ph_token, style_embed, 
                    mel2ph, f0, uv, infer=infer,global_step=global_step)
        if(ifreturn):
            return ret
        nonpadding = (ret['mel2ph'] != 0).float().unsqueeze(1).unsqueeze(1) # [B, T]
        cond = ret['decoder_inp'].transpose(1, 2)
...
class NS2_TTS_Diff(nn.Module):
    def __init__(self,phone_encoder, out_dims, denoise_fn,
                 timesteps=1000, time_scale=1, loss_type='l1', betas=None, spec_min=None, spec_max=None):
        super().__init__()
        self.denoise_fn = denoise_fn
        self.vc_clap_encoder = TTS_Seq_Network(len(phone_encoder),hparams, out_dims)
        self.mel_bins = out_dims
    ...
    @torch.no_grad()
    def p_sample(self, x_t, t, cond, prompt, spk_emb=None, clip_denoised=True, repeat_noise=False):
        b, *_, device = *x_t.shape, x_t.device
        x_0_pred = self.denoise_fn(x=x_t,times=t,prompt=prompt,cond=cond)
    ...
    def forward(self, mel, ref_mel, ph_token, style_embed, mel2ph=None,
        ...
        if not infer:
            ...
            # Predict x_{start}
            x_0_pred = self.denoise_fn(x=x_t,times=t,prompt=prompt,cond=cond) * nonpadding
            ...
            x = torch.randn(shape, device=device)  # noise
            for i in tqdm(reversed(range(0, t)), desc='ProDiff Teacher sample time step', total=t):
                x = self.p_sample(x, torch.full((b,), i, device=device, dtype=torch.long), cond,prompt)  # x(mel), t, condition(phoneme)
            x = x[:, 0].transpose(1, 2)
            ret['mel_out'] = self.denorm_spec(x)  # 去除norm
        return ret

class Codec_WavLM_TTS_Diff(nn.Module):
    def __init__(self,phone_encoder, out_dims, denoise_fn,
                 timesteps=1000, time_scale=1, loss_type='l1', betas=None, spec_min=None, spec_max=None):
        super().__init__()
        self.denoise_fn = denoise_fn
        self.ttc_encoder = Codec_WavLM_Network(len(phone_encoder),hparams, out_dims)
        self.mel_bins = out_dims
    ...
    def forward(self, mel, ref_mel, ph_token, style_embed,wavlm_spk_embed, mel2ph=None,
                f0=None, uv=None, infer=False,ifreturn=False,global_step=0):
        b, *_, device = *ph_token.shape, ph_token.device
        target = mel
        ret = self.ttc_encoder(mel, ref_mel, ph_token, style_embed, wavlm_spk_embed,
                    mel2ph, f0, uv, infer=infer,global_step=global_step)
        if(ifreturn):
            return ret
        nonpadding = (ret['mel2ph'] != 0).float().unsqueeze(1).unsqueeze(1) # [B, T]
