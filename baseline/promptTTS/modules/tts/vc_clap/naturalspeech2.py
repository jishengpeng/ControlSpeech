...
class CroAttnFilMTransformer(nn.Module):
    def __init__(
        self,
        dim,
        *,
        ...
        for _ in range(depth):
            ...
            # self.to_gamma_beta(cond).chunk(2, dim = -1)
            self.film_layers.append(nn.Linear(dim,2*dim))
    ...
    def forward(
        ...
        for (attn_norm, attn, cross_attn_norm, cross_attn, ff_norm, ff),film in zip(self.layers,self.film_layers):
            res = x
            x = attn_norm(x, cond = t)
            x = attn(x) + res
            ...
            if exists(cross_attn):
                ...
                res = x
                x = cross_attn_norm(x, cond = t)
                ### attention的结果，还没有加到query查询的结果上
                x = cross_attn(x, context = context)
                gamma, beta = film(x).chunk(2, dim = -1)
                x = res * gamma + beta
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.block = mlist([
            rmsnorm(dim),
            Attention(dim = dim, dim_head = dim_head, heads = heads, dropout = dropout, use_flash = use_flash),
            rmsnorm(dim) if cross_attn else None,
            Attention(dim = dim, dim_head = dim_head, heads = heads, dropout = dropout,use_flash = use_flash) if cross_attn else None,
            rmsnorm(dim),
            FeedForward(dim = dim, mult = ff_mult, causal_conv = ff_causal_conv)
        ])
    ...
    def forward(
        ...
        attn_norm, attn, cross_attn_norm, cross_attn, ff_norm, ff = self.block
        res = x
        x = attn_norm(x, cond = t)
        x = attn(x) + res
...
class DurationPitchFilmPredictor(nn.Module):
    def __init__(
        self,
        dim = 512,
        depth = 10,
    ...
    def forward(
        ...
        for (conv, norm, attn),film in zip(self.layers,self.film_layers):
            x = conv(x)
            attn_res = attn(norm(x), encoded_prompts, mask = prompt_mask)
            gamma, beta = film(attn_res).chunk(2, dim = -1)
            x = x * gamma + beta
            ### 原来方案的是直接将cross attention的结果加到原来的latent embedding上
            # x = attn_res + x   

        return self.to_pred(x)


class InstructCondTransformerEncoder(nn.Module):
    def __init__(self,
                 n_vocab,
                 in_channels,
                 out_channels,
    ...
    def forward(self, x, cond, x_mask=None):
        if self.n_vocab > 0:
            x = self.emb(x) * math.sqrt(self.in_channels)  # [b, t, h]
        x = torch.transpose(x, 1, -1)  # [b, h, t]
        # x_mask = torch.unsqueeze(sequence_mask(x_lengths, x.size(2)), 1).to(x.dtype)
        ...
        x = x.transpose(1,2)  # [B,T,C]
        x_mask = x_mask.transpose(1,2)
        x = self.encoder(x, times=cond) * x_mask
        return x

class InstructAttentiveStatsPool(nn.Module):
    def __init__(self,in_dim=192,out_dim=192):
        super().__init__()
        self.pooling = AttentiveStatsPool(in_dim, attention_channels=32, global_context_att=False)
...
class Style_Decoder(nn.Module):
    ...
    def forward(
        ...
        # x (B,C,T) mask (B,1,T) prompt (B,C,N) times (B,)
        x = x.transpose(1,2)
        ...
        x = self.transformer(x, t, context = c)    # (B,T,C)
...
class WavLMSpeakerEncoder(nn.Module):
    def __init__(self,spk_bottleneck_dim=128,hidden_size=192, depth=2,num_streams=2):
        ...
        self.spk_bank = StyleBankExtractor(mid_dim,2,4,hidden_size,2)
    ...
    def forward(self,mel):
        ...
        for stream in self.transformer_streams:
            x = stream(x=spk_embed,mask=mask)
            out += x
        spk_embed, attn_weights = self.spk_bank(out.transpose(0,1),padding_mask=mask)
        
        return spk_embed  # [B,8,C]
...
# )
# x = torch.rand((16,256,126))
# t = torch.randint(0, 10, (16,)).long()
# print(t.shape)
# prompt = torch.rand((16,256,36))   # [B,N,C]
# mask = None
# y = net(x,mask,prompt,t)
# print(y.shape)
