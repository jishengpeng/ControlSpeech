...
from modules.tts.ProDiff.model.diff.net import CrossAttnWaveNet


class StyleSampler(nn.Module):
    def __init__(self, timesteps=1000, time_scale=1, loss_type='l1', betas=None):
        super().__init__()
        self.denoise_fn = CrossAttnWaveNet(hparams['hidden_size'])
    ...
    @torch.no_grad()
    def p_sample(self, x_t, t, prompt, cond, clip_denoised=True, repeat_noise=False):
        b, *_, device = *x_t.shape, x_t.device
        x_0_pred = self.denoise_fn(x_t,t,cond,prompt)
    ...
    def forward(self, prosody_code, prompt,seq_T,ret,cond=None,infer=False):
        b, *_, device = *prompt.shape, prompt.device
        
        target = prosody_code   # (B,T,C)
        # prompt (B,N,C)
        
        # nonpadding = ((style_tgt_code.abs().sum(dim=-1))>0).float()  # (B,T)
        # cond = ret['decoder_inp'].transpose(1, 2)
        if not infer:
            t = torch.randint(0, self.num_timesteps + 1, (b,), device=device).long()
            # Diffusion
            x_t = self.diffuse_fn(target, t).squeeze(dim=1).transpose(1,2)   # (B,T,C)

            # Predict x_{start}
            x_0_pred = self.denoise_fn(x_t.transpose(1,2), t, cond.transpose(1,2), prompt.transpose(1,2))
            x_0_pred = x_0_pred.transpose(1,2)

            ret['pro_code_pred'] = x_0_pred   # (B,T,C)
        else:
            B = prompt.shape[0]
            t = self.num_timesteps  # reverse总步数
            shape = (prompt.shape[0], seq_T, prompt.shape[2])
            ...
            for i in tqdm(reversed(range(0, t)), desc='ProDiff Teacher sample time step', total=t):
                x = self.p_sample(x, torch.full((b,), i, device=device, dtype=torch.long),prompt,cond)   
            ret['pro_code_pred'] = x.transpose(1,2)
        return ret
